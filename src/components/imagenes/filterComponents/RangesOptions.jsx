                    ...
                    className="flex flex-col grid grid-cols-3 gap-y-1">
                    {
                        Object.entries(ranges).map(([key, value]) => (
                            <button
                                ...
                                className={`hover:brightness-200 hover:bg-transparent hover:border-4 hover:border-green-700 hover:text-green-500 disabled:opacity-40 text-sm justify-center flex items-center ${value ? 'brightness-200 bg-transparent border-4 border-green-800 text-green-900' : ' border-4 border-zinc-800 bg-zinc-800'}`}
                                key={key}>
                                {key}
                            </button>
                        ))
