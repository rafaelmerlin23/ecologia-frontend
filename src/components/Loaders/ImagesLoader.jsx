...
function ImagesLoader() {
    ...
    <div className=" mt-4  p-4 grid grid-cols-2 lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-2 xl:grid-cols-4 gap-2 p-2">
        {
            range.map((index) =>(
                <div key={index} className="relative w-full h-48 bg-gray-300">
                    <ImageItemLoading />
                </div>
            ))
        }    
...
const ImageItemLoading = () => {
    return (
        <div 
            role="status" 
            className="aspect-[16/9] w-full flex items-center justify-center h-48 min-h-[12rem] bg-gray-700 animate-pulse">
            <svg 
